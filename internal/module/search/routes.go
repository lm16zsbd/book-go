...
func (r *Routes) Register(router chi.Router) {
	router.Get("/search/history", httputil.Wrap(r.Handler.GetHistory))
	router.Delete("/search/history", httputil.Wrap(r.Handler.ClearHistory))
	router.Get("/search/hot", httputil.Wrap(r.Handler.GetHot))
